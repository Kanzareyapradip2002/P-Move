import React from 'react'
import { Link } from 'react-router-dom'
import Main from './Main'


const HollywoodPage2 = () =>{
    return (
    <>
    <Main/>
    <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
    <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">20</Link></li>
    </ul>
    </nav>
    </>
    )
}
const HollywoodPage3 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage4 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage5 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage6 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage7 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage8 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage9 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage10 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage11 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage12 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div> 
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage13 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage14 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/BlackAdam">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage15 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage16 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage17 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage18 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage19 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}
const HollywoodPage20 = () =>{
  return (
  <>
  <Main/>
  <div className='Main'>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
     <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div> 
    <div className='card'>
    <Link to="">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
    <div className='card'>
    <Link to="/">
    <img className="card-img" src={require("./img/logo.jpg")} alt="" />
    </Link>
    <h6>Release: </h6>
    </div>
   </div>
  <nav aria-label="Page navigation example">
    <ul className="pagination">
    <li className="page-item"><Link className='page-link' to="/gotoHollywood">1</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage2">2</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage3">3</Link></li>
    <li className="page-item"><Link className='page-link' to="/gotoHollywoodPage4">4</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage5">5</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage6">6</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage7">7</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage8">8</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage9">9</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage10">10</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage11">11</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage12">12</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage13">13</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage14">14</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage15">15</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage16">16</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage17">17</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage18">18</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage19">19</Link></li>
    <li className="page-item"><Link className='page-link'to="/gotoHollywoodPage20">20</Link></li>
    </ul>
    </nav>
  </>
  )
}


export {HollywoodPage2,HollywoodPage3,HollywoodPage4,HollywoodPage5,HollywoodPage6,HollywoodPage7,HollywoodPage8,HollywoodPage9,HollywoodPage10,HollywoodPage11,HollywoodPage12,HollywoodPage13,HollywoodPage14,HollywoodPage15,HollywoodPage16,HollywoodPage17,HollywoodPage18,HollywoodPage19,HollywoodPage20}